...
class PolicyPage {
  ...
  renderSections() {
    ...
    const sectionsHtml = sections.map((section, index) => {
      ...
      const summaryHtml = section.summary ? this.renderSummary(section.summary) : '';
  ...
  renderSummary(summary) {
    return `
      <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-l-4 border-blue-600 rounded-r-lg px-4 py-2 mb-4 shadow-sm">
        <div class="flex items-start">
          ...
          <div class="flex-1">
            <p class="text-gray-800 font-medium leading-tight mb-0">${summary}</p>
          </div>
        </div>
      </div>
    `;
